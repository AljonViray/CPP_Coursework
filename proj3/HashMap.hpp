...
public:
    ...
    // Helper Functions
    // clearAll() resets the bucketArray to all nullptrs/no Nodes
    void clearAll();
...
private:
    ...
    Node** bucketArray;
    unsigned int numOfBuckets, hashmapSize;

    void fillWithNull(const unsigned int& numOfBuckets, Node**& bucketArray);
    void addNode(const std::string& key, const std::string& value, Node**& bucketArray, const int i);

    void rehashBucketArray(unsigned int& numOfBuckets, Node**& bucketArray);
    void copyBucketArray(const unsigned int& numOfBuckets, Node** const oldBucketArray, Node**& newBucketArray);

    void printAllNodes(const unsigned int& numOfBuckets, Node**& bucketArray) const;
};
